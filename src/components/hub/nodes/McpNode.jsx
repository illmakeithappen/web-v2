...
const NodeContainer = styled.div`
  ...
  border: 2px solid ${props => props.$selected ? carbonColors.interactive01 : '#8a3ffc'};
  border-radius: 8px;
...
function McpNode({ data, selected }) {
  ...
  return (
    <NodeContainer $selected={selected}>
      ...
      <NodeHeader>
        <NodeIcon>MCP</NodeIcon>
        <NodeTitle title={data.label || data.name}>
          {data.label || data.name || 'MCP Server'}
        </NodeTitle>
      ...
      <NodeContent>
        {data.description || data.category || 'MCP Server node'}
      </NodeContent>

      <NodeMeta>
        {data.category && <MetaBadge>{data.category}</MetaBadge>}
        <MetaBadge>MCP</MetaBadge>
      </NodeMeta>
