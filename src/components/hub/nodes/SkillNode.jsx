...
const NodeContainer = styled.div`
  ...
  border: 2px solid ${props => props.$selected ? carbonColors.interactive01 : '#24a148'};
  border-radius: 8px;
...
function SkillNode({ data, selected }) {
  ...
  return (
    <NodeContainer $selected={selected}>
      ...
      <NodeHeader>
        <NodeIcon>S</NodeIcon>
        <NodeTitle title={data.label || data.skill_name}>
          {data.label || data.skill_name || 'Skill'}
        </NodeTitle>
      ...
      <NodeContent>
        {data.description || data.skill_type || 'Skill node'}
      </NodeContent>

      <NodeMeta>
        {data.skill_type && <MetaBadge>{data.skill_type}</MetaBadge>}
