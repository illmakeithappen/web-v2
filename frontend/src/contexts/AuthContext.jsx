/**
 * Authentication Context for AWS Cognito
...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    // Check for PostgreSQL token first
    ...
    if (token && user) {
      try {
        ...
        console.log('📍 PostgreSQL auth detected, setting user:', parsedUser);
        dispatch({
          type: AUTH_ACTIONS.SET_USER,
          payload: { user: parsedUser, session: { token } }
        });
        // Don't call initializeAuth() - we have PostgreSQL auth
      } catch (error) {
        console.error('Error parsing stored user:', error);
        initializeAuth();
      }
    } else {
      // No PostgreSQL token, try Cognito
      initializeAuth();
    }
  }, []);

  // Set up auth listener (only for Cognito, skip if using PostgreSQL)
  useEffect(() => {
    // Skip Cognito listener if PostgreSQL token exists
    const token = localStorage.getItem('access_token');
    if (token) {
      console.log('📍 Using PostgreSQL auth, skipping Cognito listener');
